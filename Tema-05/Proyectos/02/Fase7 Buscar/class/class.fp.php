...
class Fp extends Conexion {
    ...
    public function getAlumnoFormat($idAlumno) {
        try {
            $stmt = "SELECT 
                al.id,
                concat_ws(\" \", al.nombre, al.apellidos) AS nombreCompleto,
                ...
                timestampdiff(YEAR, al.fechaNac, NOW()) AS edad,
                cu.nombreCorto as curso
            FROM alumnos AS al INNER JOIN cursos AS cu ON al.id_curso = cu.id WHERE al.id = :idAlumno";

            $pdostmt = $this->pdo->prepare($stmt);

            $pdostmt->bindParam(':idAlumno', $idAlumno, PDO::PARAM_INT);
    ...
    public function deleteAlumno($id) {
        try {
            $stmt = "DELETE FROM alumnos WHERE id = :id LIMIT 1";
            $pdostmt = $this->pdo->prepare($stmt);
            $pdostmt->bindParam(':id', $id, PDO::PARAM_INT);
            $pdostmt->execute();
    ...
    public function order($criterio) {
        try {
            $stmt = "SELECT 
                ...
                timestampdiff(YEAR, al.fechaNac, NOW()) AS edad,
                cu.nombreCorto as curso
            FROM alumnos AS al INNER JOIN cursos AS cu ON al.id_curso = cu.id ORDER BY :criterio";
